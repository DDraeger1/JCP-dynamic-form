...
import React, { forwardRef, useCallback, useImperativeHandle, useContext, useEffect } from "react";
...
case "paragraphValue":
  ...
  const submitDirtyFields = async (values) => {
    ...
    const valuesToSubmit = Object.entries(formState.dirtyFields).reduce(
      (acc, [key, isDirty]) => {
  ...
  const submitDirtyFieldsSuite = async (formDefinition) => {
    if (Object.keys(formState.dirtyFields).length === 0) return false;
    console.log(formDefinition)
    const valuesToSubmit = formDefinition
